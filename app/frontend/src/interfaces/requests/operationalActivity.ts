...
export interface OperationalActivityDepartmentAttributes {
  id: number;
  title: string;
...
export interface CreateOperationalActivityBody extends Omit<OperationalActivityAttributes, 'id' | 'createdAt' | 'updatedAt'> {
  // Все поля опциональны кроме обязательных
}

// Ответ при создании операционной деятельности
export interface CreateOperationalActivityResponse {
  operationalActivity: OperationalActivityAttributes;
}
