...
export const updateEvent = asyncErrorHandler(
  async (req: Request, res: CustomResponse) => {
    ...
    const result = await sequelize.transaction(async (transaction) => {
      ...
      await eventPunishmentService.deleteEventPunishmentByEventId(Number(id), { transaction });
      ...
      if (data.criminal_case) {
        ...
        await eventCriminalCaseService.createEventCriminalCase(
          { ...criminalCaseWithoutId, event_id: Number(id) },
          { transaction }
        );
      }
    ...
    res.success(result, 'Event updated successfully');
  }
);
